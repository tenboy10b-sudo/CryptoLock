...
**Credential warning (confirmed 2026-09-25):** the existing Production `GITHUB_TOKEN` (used by `pages/api/autopost.js` for the public `CryptoLock` repo) was tested against `CryptoLock-analytics` via a temporary server-side diagnostic and **cannot read it** (`repo_access: false`) — it's most likely scoped only to the public repo. Do not assume `GITHUB_TOKEN` will work for the analytics repo without first re-testing; a dedicated least-privilege credential scoped to `CryptoLock-analytics` is needed before the writer can be implemented. `GITHUB_TOKEN` is also configured as a Vercel "Sensitive" env var, so its value can never be read locally (`vercel env pull` returns it empty) — any future credential test for it must run server-side (a temporary deployed endpoint), not locally.
