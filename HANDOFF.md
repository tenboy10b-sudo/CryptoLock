...
cron-job.org job "CryptoLock TikTok Daily Analytics" — daily 03:15 UTC, ENABLED
  (configured per the account owner; scheduler TEST RUNs verified live 2026-09-26;
   first natural clock-triggered run NOT yet verified)
  ...
  → write/update the private daily snapshot: tenboy10b-sudo/CryptoLock-analytics
      tiktok/snapshots/YYYY/MM/YYYY-MM-DD.json (ANALYTICS_GITHUB_TOKEN; create verified
      live 2026-09-25 and 2026-09-26, same-day update verified live 2026-09-26)
  → secret-gated JSON response (no token/secret values)

Manual mode is still available: the account owner can call the same endpoint by hand
with TIKTOK_COLLECT_SECRET (that is how the first runs were done, before the cron job).
```
