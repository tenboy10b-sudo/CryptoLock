---
title: "Обмеження кількості спроб пароля Windows: GPO і реєстр"
date: "2026-04-08"
updated: "2026-05-23"
description: "Встанови обмеження спроб пароля через GPO або реєстр. Захист від brute-force атак на Windows 10 і 11 — покроково за 3 хвилини."
...
### Скільки спроб пароля оптимально встановити?

Рекомендація Microsoft і NIST — 5-10 спроб. Занадто мало (1-3) призводить до частого блокування реальних користувачів. Занадто багато (20+) не захищає від brute-force.

### Чи можна налаштувати без GPO на Windows Home?

Так, через команду: `net accounts /lockoutthreshold:5`. Або через secpol.msc якщо доступний. Обидва методи працюють без gpedit.

### Що робити якщо адміністратор сам себе заблокував?

Перезавантаж в Safe Mode — вбудований Administrator не блокується за замовчуванням. Або скинь лічильник з іншого адміністраторського акаунту: `net user Ім'я /active:yes`.

### Чи захищає від мережевих атак через RDP?

Так, але тільки для локальних акаунтів і при доступі через SMB/RDP. Для повного захисту додатково обмеж RDP доступ по IP через Windows Firewall.
