...
description: "Як налаштувати двофакторну аутентифікацію для входу в Windows через Microsoft Authenticator, TOTP, Windows Hello і апаратні ключі FIDO2. Примусова 2FA через Azure AD і резервні методи відновлення."
...
Або в Azure Portal: **Azure AD** → **Security** → **Authentication methods** → **FIDO2 security keys** → увімкни.
...
**Якщо втратив пристрій з 2FA:** використай резервний код або SMS на резервний номер. Якщо нічого немає — **account.microsoft.com** → **Безпека** → **Не маю доступу до цих параметрів** → процедура відновлення акаунту (займає 24-30 днів).
