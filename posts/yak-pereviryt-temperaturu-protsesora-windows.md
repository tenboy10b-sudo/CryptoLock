...
description: "Як переглянути температуру CPU і GPU в Windows через вбудовані засоби, HWiNFO64, Core Temp, MSI Afterburner і PowerShell. Норми температур для десктопу і ноутбука, що робити при перегріві."
...
## Core Temp — легка альтернатива тільки для CPU

Якщо HWiNFO64 здається завеликим — [Core Temp](https://www.alcpu.com/CoreTemp/) показує температуру кожного ядра CPU прямо в треї, без зайвих вікон.

При встановленні обов'язково зніми галочку з пропонованого додаткового ПЗ.
...
|-----------|---------------|---------------------|---------|
| CPU (десктоп) | 30–50°C | 70–85°C | >95°C |
| CPU (ноутбук) | 35–50°C | 75–90°C | >100°C |
| GPU | 30–45°C | 70–85°C | >90°C |
| SSD NVMe | 30–50°C | 60–70°C | >80°C |
| HDD | 25–40°C | 40–50°C | >55°C |

Ноутбуки працюють гарячіше десктопів при тому самому навантаженні — менше місця для радіаторів і повітря. Сучасні процесори при досягненні критичної температури самі знижують частоту (throttling) або вимикають ПК, щоб не згоріти.
...
### Для ноутбука — додатково

- Поклади на тверду рівну поверхню, не на ліжко чи диван — м'яка поверхня перекриває вентиляційні отвори знизу
- Почисти вентиляційні отвори стисненим повітрям (без розбирання корпусу)
- Використовуй підставку з активним охолодженням при тривалому навантаженні (ігри, рендеринг)
- Заміна термопасти на ноутбуці складніша ніж на десктопі — якщо немає досвіду розбирання, краще звернутись у сервіс
