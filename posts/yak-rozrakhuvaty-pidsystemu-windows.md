...
## Як обрати адресу для статичного IP

- Вибирай адресу поза діапазоном DHCP роутера (зазвичай `.100`-`.200`)
- Для статичних адрес: `.2` до `.99`
- Перевір реальний діапазон DHCP в адмін-панелі роутера
...
Ні. Статичний vs DHCP — тільки спосіб призначення адреси, не впливає на швидкість чи затримку.

### Статичний IP перестав працювати після скидання роутера?

Скидання роутера часто змінює IP шлюзу (повертається до 192.168.0.1 або 192.168.1.1 за замовчуванням). Онови шлюз у налаштуваннях статичного IP відповідно до нової адреси роутера.
