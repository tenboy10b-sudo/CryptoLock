...
## Скільки місця потрібно на новому диску

Новий диск має бути **не менший за фактично зайнятий простір**, а не за весь обсяг старого. Приклад: на HDD 500 ГБ, але зайнято лише 80 ГБ — SSD на 120 ГБ підійде.

Перевір скільки зайнято: `Win + E` → **Цей комп'ютер** → правою кнопкою на диск C → **Властивості**.

Якщо зайнято забагато для нового диска — спочатку почисти: [Як почистити диск C](/yak-pochystyty-dysk-c)
...
Або через GUI: `Win + X` → **Керування дисками** → правий клік на розділі C → **Розширити том**.

### Windows зовсім не завантажується після заміни диска

Завантажся з інсталяційного USB → **Відновлення системи** → **Відновлення при завантаженні**. Якщо не допомогло — команди `bootrec` вище. Детальніше: [Відновлення завантажувача Windows](/vidnovlennya-zavantazhuvach-windows)

---

## Що робити зі старим диском

Не викидай — він стає зовнішнім сховищем:

- Відформатуй через **Керування дисками**
- Постав у зовнішній USB-корпус (caddy) і використовуй для бекапів, відео, ігор

---

## Результат

Типове прискорення після переходу з HDD на SSD:

- Завантаження Windows: з 60–120 секунд до 10–20 секунд
- Відкриття браузера: з 5–10 секунд до 1–2 секунд
- Відкриття Word / Excel: практично миттєво

Перевір з якого диска фактично завантажилась система:

```powershell
Get-PhysicalDisk | Select-Object FriendlyName, MediaType, Size
```

Після переходу на SSD варто ще й оптимізувати систему під нього: [Оптимізація SSD в Windows](/optymizatsiya-ssd-windows)

---

## Підсумок

Macrium Reflect Free → Clone this disk → вибери старий і новий диск → розшир розділ якщо потрібно → Finish. Після клонування — змінити порядок завантаження в BIOS, перевірити TRIM (`fsutil behavior query DisableDeleteNotify`) і активацію Windows (`slmgr /xpr`).
