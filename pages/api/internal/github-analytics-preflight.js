// TEMPORARY diagnostic endpoint — verifies the dedicated
// ANALYTICS_GITHUB_TOKEN (never the old GITHUB_TOKEN) can read the private
// tenboy10b-sudo/CryptoLock-analytics repo before the TikTok analytics
// snapshot writer is implemented. Read-only: never creates, updates, or
// deletes any file in that repo. Removed immediately after use — see
// CryptoLock DOCUMENTATION.md for the dated record of this preflight.
// Never returns/logs/hashes/prefixes ANALYTICS_GITHUB_TOKEN or
// ANALYTICS_PREFLIGHT_SECRET.
...
export default async function handler(req, res) {
  ...
  // Deliberately ONLY ANALYTICS_GITHUB_TOKEN — no fallback to GITHUB_TOKEN,
  // which already failed this same preflight for this repo.
  ...
  const ok = repoAccess && isPrivate === true && defaultBranchIsMain === true && readAccess && pullPermission && writePermission
