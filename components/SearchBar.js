...
function DesktopSearch({ index, loadIndex, isEn, locale }) {
  ...
  const onKeyDown = e => {
    if (e.key === 'ArrowDown') { e.preventDefault(); setActive(a => Math.min(a + 1, results.length - 1)) }
      ...
      window.location.href = (locale === 'en' ? '/en/' : '/') + results[active].slug
    }
  }
  ...
  return (
    <div ref={wrapRef} style={d.wrap}>
      <div style={{ ...d.box, ...(open ? d.boxOpen : {}) }}>
        ...
        />
        {query && (
          <button onClick={() => { setQuery(''); inputRef.current?.focus() }} style={d.clearBtn} tabIndex={-1}>
            <CloseIcon />
          </button>
        )}
      </div>
      ...
      {show && (
        <div style={{
          ...
          {results.length > 0 ? results.map((post, i) => (
            ...
            <div style={d.empty}>
              {isEn ? `No results for "${query}"` : `Нічого не знайдено за «${query}»`}
            </div>
          )}
          {results.length > 0 && (
            <div style={d.footer}>
              {results.length} {isEn ? `result${results.length > 1 ? 's' : ''}` : `результат${results.length > 1 ? 'и' : ''}`} · ↑↓ · Enter
            </div>
          )}
        </div>
      )}
...
const d = {
  ...
  itemDesc: { display:'block', fontSize:'11px', color:'var(--muted,#64748b)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', marginTop:'2px' },
