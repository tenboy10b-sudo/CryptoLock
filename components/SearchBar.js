...
const m = {
  ...
  box:     { display:'flex', alignItems:'center', gap:'8px', flex:1, background:'var(--bg,#f1f5f9)', borderRadius:'10px', padding:'0 12px', height:'42px', minWidth:0 },
  boxIcon: { color:'var(--faint,#94a3b8)', display:'flex', flexShrink:0 },
  input:   { flex:1, border:'none', outline:'none', background:'transparent', fontSize:'16px', fontFamily:'var(--font-body)', color:'var(--text,#0f172a)', minWidth:0, width:'100%' },
  clearBtn:{ display:'flex', alignItems:'center', background:'none', border:'none', cursor:'pointer', color:'var(--faint,#94a3b8)', padding:'4px', flexShrink:0 },
  cancel:  { background:'none', border:'none', cursor:'pointer', fontSize:'14px', fontWeight:500, color:'var(--accent,#2563eb)', padding:'0 2px', whiteSpace:'nowrap', flexShrink:0 },
  body:    { flex:1, overflowY:'auto', overflowX:'hidden' },
  hint:    { padding:'2rem 1rem', textAlign:'center', fontSize:'13px', color:'var(--faint,#94a3b8)' },
  empty:   { padding:'3rem 1rem', textAlign:'center' },
  emptyTitle: { fontSize:'15px', fontWeight:500, color:'var(--text,#334155)', margin:'8px 0 4px' },
  emptyHint:  { fontSize:'13px', color:'var(--faint,#94a3b8)' },
  item:    { display:'flex', alignItems:'center', gap:'12px', padding:'14px 16px', textDecoration:'none', borderBottom:'1px solid var(--border,#f1f5f9)' },
  itemIcon:{ color:'var(--faint,#94a3b8)', display:'flex', flexShrink:0 },
  itemBody:{ flex:1, minWidth:0, display:'flex', flexDirection:'column', gap:'3px' },
  itemTitle:{ fontSize:'14px', fontWeight:500, color:'var(--text,#0f172a)', lineHeight:1.4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', display:'block' },
  itemDesc: { fontSize:'12px', color:'var(--muted,#64748b)', lineHeight:1.4, display:'block' },
...
const d = {
  ...
  dropdown:{ position:'absolute', top:'calc(100% + 6px)', left:0, right:0, background:'var(--bg-card,#fff)', border:'1px solid var(--border,#e2e8f0)', borderRadius:'12px', boxShadow:'0 8px 32px rgba(15,23,42,0.12)', zIndex:100, overflow:'hidden' },
  item:    { display:'flex', alignItems:'center', gap:'10px', padding:'10px 14px', textDecoration:'none', borderBottom:'1px solid var(--border,#f8fafc)', transition:'background .1s' },
  itemActive:{ background:'var(--accent-light,#eff6ff)' },
  itemIcon:{ color:'var(--faint,#94a3b8)', flexShrink:0, display:'flex' },
  itemBody:{ flex:1, minWidth:0 },
  itemTitle:{ display:'block', fontSize:'13px', fontWeight:500, color:'var(--text,#0f172a)', lineHeight:1.35, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' },
  itemDesc: { display:'block', fontSize:'11px', color:'var(--muted,#64748b)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' },
  arrow:   { fontSize:'18px', color:'var(--border-md,#cbd5e1)', flexShrink:0 },
  footer:  { padding:'7px 14px', background:'var(--bg,#f8fafc)', fontSize:'11px', fontFamily:'var(--font-mono)', color:'var(--faint,#94a3b8)' },
  empty:   { padding:'1.25rem 14px', fontSize:'13px', color:'var(--faint,#94a3b8)', textAlign:'center' },
}

const sb = {
  btn: { display:'flex', alignItems:'center', justifyContent:'center', width:'36px', height:'36px', borderRadius:'8px', background:'var(--bg,#f1f5f9)', border:'1px solid var(--border,#e2e8f0)', cursor:'pointer', color:'var(--muted,#475569)', flexShrink:0 },
}
