...
import { validateTokenBundle, hasRequiredScopes } from './tiktokTokenStore'
...
// Refresh-specific validation on top of the shared structural check (which
// already rejects a non-Bearer token_type and an invalid open_id type): the
// refreshed bundle must still carry both required scopes, and open_id must be
// continuous with the previously stored bundle — TikTok refreshing to a
// different account's token, or silently dropping open_id, would be a
// serious, silent bug if not caught here.
export function validateRefreshedBundle(tokenJson, previousOpenId) {
  const base = validateTokenBundle(tokenJson)
  if (!base.ok) return base

  if (!hasRequiredScopes(base.bundle.scope)) {
    return { ok: false, reason: 'insufficient_scope' }
  }

  // If the previous bundle had a real open_id, the refreshed one must match it
  // exactly — including rejecting a refresh that comes back with open_id: null,
  // which the old `&&`-based check below used to silently let through:
  //   previousOpenId && base.bundle.open_id && previousOpenId !== base.bundle.open_id
  // If the previous open_id was null, any value validateTokenBundle already
  // accepted (null or a valid non-empty string) is fine.
  if (previousOpenId !== null && previousOpenId !== undefined) {
    if (base.bundle.open_id !== previousOpenId) {
