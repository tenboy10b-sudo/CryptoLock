...
export function getAllPosts(locale = 'uk') {
  if (locale !== 'en' || !fs.existsSync(postsEnDirectory)) {
    ...
    return fs.readdirSync(postsDirectory)
      ...
      .filter(isPublished)
      .sort((a, b) => ((a.updated || a.publishDate || a.date || '') < (b.updated || b.publishDate || b.date || '') ? 1 : -1))
  }

  // Англійська: тільки EN статті
  return fs.readdirSync(postsEnDirectory)
    .filter(f => f.endsWith('.md'))
    .map(fileName => {
      const slug = fileName.replace(/\.md$/, '')
...
export async function getPostBySlug(slug, locale = 'uk') {
  ...
  return { slug, contentHtml: processed.toString(), ...data, isFallback }
}
