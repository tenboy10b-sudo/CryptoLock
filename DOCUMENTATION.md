...
**Дія:** оскільки OAuth-блок на GitHub-акаунті щойно знято (підтверджено користувачем), спробували перепідключити Vercel↔GitHub напряму через Dashboard — крок, що раніше впирався в порожню 404-сторінку встановлення GitHub App. Цього разу GitHub App вже бачив репозиторій `CryptoLock` в списку — користувач натиснув **Connect**.
...
- [ ] Перевірити чи Vercel↔GitHub зв'язок реально відновлено (тестовий коміт → чи задеплоїться сам)
- [ ] Якщо так — розглянути вимкнення нічного Railway full-rebuild cron, повернутись до природних git-тригерних деплоїв
