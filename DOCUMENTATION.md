...
#### Фікс 2 — зайві JS-поліфіли для старих браузерів (13 КіБ)

**Причина:** в проєкті не було файлу `browserslist` — Next.js транспілював код під дуже старий набір браузерів за замовчуванням, додаючи непотрібні поліфіли (`Array.prototype.at/flat/flatMap`, `Object.fromEntries/hasOwn`, `String.prototype.trimStart/trimEnd`), які нативно підтримуються всіма сучасними браузерами.

**Виправлено:** додано в `package.json` секцію `browserslist` з офіційно рекомендованим Next.js "сучасним" таргетом (`chrome 64, edge 79, firefox 67, opera 51, safari 12`) — це прибирає непотрібну транспіляцію.

Обидва фікси — коміт `c7500bf`, задеплоєно, перевірено на живому сайті.

**TODO:** переперевірити PageSpeed через кілька днів (кеш-заголовок сам по собі майже не впливає на бал "Ефективність" при першому скануванні, ефект більше для повторних відвідувань; поліфіли — трохи зменшать розмір JS).
