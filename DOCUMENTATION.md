...
#### НЕ З'ЯСОВАНО — розбіжність між git-історією `published.json` і реальним постингом

**Що показує git:** коміти `bot: update published.json [...]` йшли регулярно щодня (кілька разів на день) з 4 по 10 липня, останній — **10.07.2026 13:01** (`677e972`). Після цього — **жодного коміту `published.json` за 5 днів** (11-15 липня), хоча має спрацьовувати за розкладом (`content` 10:00, `middle` 13:00, `extra` 15:00 через cron-job.org).

**Що повідомив користувач:** пости в Telegram-каналі йдуть за розкладом, постинг є.

**Ці два факти не перевірені разом до кінця** — можливі пояснення (жодне не підтверджено): (1) сам `sendMessage`/`sendPoll` в Telegram відпрацьовує успішно, а окремий крок запису `published.json` через GitHub Contents API мовчки падає (наприклад, через конфлікт `sha` при паралельних викликах або тимчасову проблему GitHub API) — у такому разі бот міг би з часом почати повторно публікувати вже опубліковані статті, бо втратив трек історії; (2) або зміни таки пишуться, але кудись не в той репозиторій/гілку, яку перевіряв `git log`; (3) або сталася ще не зрозуміла причина. **Не досліджено в цій сесії** — перервано користувачем, перепрiоритезовано на GSC-перевірку. Дивитись `vercel logs` для `/api/autopost` в наступній сесії, щоб знайти справжню причину.
...
- [ ] **З'ясувати розбіжність автопостингу** — чому `published.json` не оновлювався в git з 10.07, хоча пости в Telegram йдуть за розкладом (дивитись `vercel logs`, перевірити чи не падає мовчки крок запису в GitHub)
