...
- **Тарифи продажу клієнтам** (дохід, не витрата): AuditShield $9/13/22, StaffSpy $9/15/22 — картка Monobank + USDT TRC-20 (реквізити оплати — операційні/приватні й не мають зберігатися в публічному репозиторії), один і той самий гаманець/картка для обох ботів
...
### Сесія 12 (продовження 43) — Взаємна перелінковка 6 пар + оцінка "тонкого контенту" за запитом користувача

Користувач запитав: скільки статей ще не вийшли і яка остання дата; чи є сенс дописувати короткі статті (стара знахідка — медіана ~429 слів, 44% коротші за 400) для індексації.

**Пораховано точно:** 98 статей ще не опубліковано (49 UK + 49 EN, попарно), перша дата 17.08.26, остання — 6.10.26, розподіл 26/60/12 по місяцях (серпень/вересень/жовтень) — природний темп ~1 пара на день.

**Рекомендація щодо тонкого контенту (не виконано, лишено на розсуд):** відкласти до контрольної точки GSC. Аргумент — довжина сама по собі не причина (Google не має мінімуму слів), шкодить саме комбінація короткий+шаблонний/дублюючий, а дублюючу частину щойно прибрали. Дописування похапцем ризикує відтворити той самий патерн "штучно роздутого" контенту. Якщо після чистки дублікатів індексація не зрушить — тоді це стає обґрунтованою наступною гіпотезою.

**Зроблено (низький ризик, тільки додавання):** взаємна перелінковка між 6 парами статей, які цієї сесії свідомо залишені окремими (не дублікати, але споріднені): tuning мережевого адаптера ↔ загальна конфігурація, мережеві шари (хост) ↔ підключення диска (клієнт), Performance Monitor deep-dive ↔ загальний огляд інструментів моніторингу, чорний екран загальний ↔ після входу, дитячий ПК через Family Safety ↔ через локальний акаунт, помилки Print Spooler ↔ "принтер не друкує" (і UK, і EN сторони). По одному контекстному реченню в кожен бік, нічого не видалено чи об'єднано.

**Перевірено:** build успішний, усі 14 файлів з посиланнями існують, живо перевірено рендер посилання й відповідні сторінки. Задеплоєно вручну (`git push` з першої спроби впав через тимчасову 500-ку GitHub, з другої пройшов).

#### Оновлений TODO

- [ ] Слідкувати за відповіддю GitHub Support на звернення про акаунт
- [ ] Пам'ятати деплоїти вручну (`vercel --prod`) після кожного пуша, поки git-зв'язок не відновиться
- [ ] Контрольна точка GSC — кінець серпня — початок вересня (тоді ж вирішити чи братись за розширення тонкого контенту)
- [ ] Спостерігати за природним виходом запланованих статей (17 серпня — 6 жовтня, 98 статей)

---

### Сесія 12 (продовження 44) — Знайдено третю статтю про реєстр через реальну історію Telegram-каналу

Користувач надав повний HTML-експорт історії каналу CryptoLock (353 повідомлення, "мені вроді є дуже схожі статті") — новий, незалежний від title/description/H2 сигнал: реальні статті, які бот коли-небудь реально просував.

**Метод:** витягнуто текст усіх повідомлень, знайдено всі унікальні посилання на статті сайту (25 унікальних slug — небагато, бо посилання додається лише в кожному 4-му "content"-пості за логікою бота). Більшість "схожих" заголовків у самих постах виявились фіксованим списком тем для engagement/extra-постів (`EXTRA_STYLES`/`ENGAGE_STYLES` в `autopost.js`), що навмисно рециклюється — не мають стосунку до статей сайту, хибний сигнал.

**Реальна знахідка:** `redahuvannya-reyestru-windows` (UK) — **третя** стаття про редагування реєстру, яку title/description-скан пропустив (стемінг "редагування реєстру" не перетнувся з "редактор реєстру" в canonical). Її EN-пара `how-to-configure-remote-registry-windows.md` виявилась ЩЕ й розсинхронізованою слаг↔вміст — назва каже "remote registry" (віддалений реєстр), а вміст — про звичайне локальне редагування через regedit/PowerShell, жодної згадки remote. Об'єднано в `yak-pratsyuvaty-z-reiestratorom-windows`, EN-файл перейменовано на `how-to-edit-windows-registry`, вперше зв'язано `translatesUk`/`translatesEn` в обидва боки (раніше не було жодного зв'язку).

**Побічна цінність перевірки:** решта ~20 посилань з каналу або досі існують як окремі легітимні статті, або вже виявились коректно редиректнутими попередніми злиттями цієї сесії (won't-boot, mojibake 2FA, моніторинг мережі) — незалежне підтвердження що ті злиття були правильними й повними.

**Перевірено:** build успішний, обидва старі слаги → 308 → 200, головна ціла. Задеплоєно вручну.
...
### Сесія 12 (продовження 45) — Автоматичний SEO-моніторинг (GSC+PageSpeed) + фікс CLS через шрифти + уповільнення черги публікацій до 2 пар/тиждень

Три окремі блоки роботи за сесію.

#### 1. Побудовано автоматичний щотижневий SEO-моніторинг — окремо від старого `gsc_analyzer.py`

За зразком уже робочого рішення для інших проєктів (TileDash/FlowCraft) — новий окремий GCP-проєкт `ryptolockua-seo-monitor` (Project ID втратив першу літеру "c" при створенні через Cloud Console, некритично — скрипт бере лише email service account'а з JSON, ніде не хардкодить project ID), service account `gsc-reader` доданий у Search Console (Restricted), окремий обмежений PageSpeed Insights API-ключ.

Секрети — поза репо сайту: `C:\Users\rr\Desktop\cryptolockua-seo-monitor\secrets\`. Скрипт `gsc_report.py` (адаптований з шаблону tiledash-seo-monitor) — на відміну від шаблону, **не хардкодить список URL для перевірки індексації**, а читає `posts/`/`posts-en/` живого репо сайту при кожному запуску, фільтрує вже опубліковане, бере 15 найновіших з кожної мови. UK і EN звітуються окремо (трафік, індексація) — двомовний сайт, усереднення ховає яка сторона реально проблемна.

Автоматизація — локальна Windows Scheduled Task "CryptolockUA SEO Report", щонеділі 10:00 (той самий ритм, що був у старому GitHub Actions SEO-звіті). Перевірено наскрізь: ручний запуск успішний, тестовий запуск САМЕ через Task Scheduler (не інтерактивний термінал) — теж успішний (`LastTaskResult: 0`). Перший звіт: `cryptolockua-seo-monitor/reports/2026-08-19.md`.

**Перше реальне спостереження:** усі 30 найновіших статей (15 UK + 15 EN) — "URL is unknown to Google", жодного разу не скановані. Узгоджується з уже задокументованою картиною (сайт-вайд недовіра), не новий сигнал.

#### 2. Знайдено й виправлено джерело високого CLS через веб-шрифти

Перевірка PageSpeed-деталей (audit `layout-shifts`, не просто CLS-число) для двох сторінок з першого звіту (CLS 0.55 і 0.41) показала точну причину: `pages/_document.js` підключає Google Fonts (Inter/Unbounded) з `display=swap` — текст спершу рендериться шрифтом-заступником, потім Lighthouse прямо фіксує `Web font loaded` як причину зсуву, коли текст перемальовується справжнім шрифтом з іншою метрикою символів. Найбільше зачіпає сторінки з великим текстовим блоком біля початку (вступний абзац/тіло статті) — на головній (менше суцільного тексту одразу під шапкою) CLS 0.004, нормально.

**Фікс:** `display=optional` замість `display=swap` в `pages/_document.js` — браузер або встигає завантажити шрифт вчасно, або лишається на заступнику без подальшого перемальовування. Коміт `a096051`, задеплоєно вручну, перевірено живо (`display=optional` в HTML, обидві сторінки 200). Ефект на CLS буде видно в наступному щотижневому звіті (23.08).

#### 3. Черга неопублікованих статей уповільнена з ~1 пари/день до 2 пар/тиждень

За проханням користувача, після обговорення що падіння "Проіндексовано" (66→44 за тиждень) — здебільшого наслідок здорового скорочення пулу sitemap через консолідацію (858→461 URL), а не нова проблема, і форсування індексації відхилено (той самий принцип, що й раніше — масові дії в GSC історично шкодили, 13 червня). Темп публікацій — окремий, додатковий важіль обережності (швидкість появи контенту — один із сигналів scaled-content-abuse систем, не тільки дублювання).

Написано `scripts/respace_queue.py` — читає всі файли з `publishDate` в майбутньому, пари UK↔EN через `translatesEn`/`translatesUk` (+ 3 "солов" — де переклад уже опублікований раніше, теж отримали власний слот у тому самому графіку), зберігає відносний порядок, розставляє на новий графік Вт+Пт (2 події/тиждень). Спершу прогнано в dry-run режимі, перевірено план, потім застосовано до 93 файлів (45 пар × 2 + 3 солов). Новий діапазон виходу: 21.08.26 — 02.02.27 (було 17.08–06.10). `date`/`publishDate` синхронізовані в кожному файлі (той самий баг, що вже ловили в продовженні 38, тут запобіжно перевірено — 0 розбіжностей).

**Перевірено:** build успішний, коміт `4b061b4`, задеплоєно вручну, живо — обидві тестові статті (нова найближча дата 21.08 і 25.08) коректно повертають 404 (ще не час), головна ціла.

#### Оновлений TODO

- [ ] Слідкувати за відповіддю GitHub Support на звернення про акаунт (тікет #4661396, чекає відповіді на питання Ivy про мету використання)
- [ ] Пам'ятати деплоїти вручну (`vercel --prod`) після кожного пуша, поки git-зв'язок не відновиться
- [ ] Контрольна точка GSC — кінець серпня — початок вересня
- [ ] Наступний щотижневий SEO-звіт — 23.08 (перевірити чи знизився CLS на двох статтях, чи почала рухатись індексація)
- [ ] Спостерігати за природним виходом запланованих статей за НОВИМ графіком (21 серпня 2026 — 2 лютого 2027, 2 пари/тиждень)

---

### Сесія 12 (продовження 46) — Знайдено й виправлено повторення тем у Telegram extra-постах через новий Telegram-експорт

Користувач надав свіжий HTML-експорт історії каналу (`ChatExport_2026-08-29`, 402 повідомлення) — той самий метод, що знайшов приховану статтю про реєстр у продовженні 44 — з проханням проаналізувати пости з дати останніх змін (19.08) на повторення тем.

#### Знахідка — 4 чіткі повтори теми, усі рівно за 7 днів

Розпарсено 32 повідомлення з 19.08 по 29.08 (BeautifulSoup, дата+текст з кожного `div.message`). Знайдено 4 пари ідентичних/майже ідентичних заголовків, кожна пара — **рівно 7 днів між появами**, обидва рази опубліковані **рівно о 19:01**: "Чому Windows займає багато місця на диску" (19.08/26.08), "CMD vs PowerShell" (20.08/27.08), "Щомісячне обслуговування ПК" (21.08/28.08), "Диск C: переповнений" (22.08/29.08).

**Перевірено й підтверджено — це НЕ повторення проблеми з продовження 44** (дублікат-стаття на сайті): жодна з 4 тем не відповідає реальному кластеру дублікатів у `posts/` (окремий греп-пошук по кожній темі — чисто). Проблема ізольована в самому Telegram-боті, не зачіпає індексацію сайту.

#### Корінна причина — знайдена в `pages/api/autopost.js`

Пости `type=extra` (щоденний слот, час не збігається із задокументованим у сесії 6 "15:00" — реальний розклад живе в cron-job.org поза репо, не звірявся) генеруються з масиву `EXTRA_STYLES` — лише 6 стилів, кожен дає Claude короткий **фіксований список прикладових тем** (6-8 пунктів) прямо в промпті. Через щоденну публікацію цикл із 6 стилів замикається щотижня — той самий короткий список тем повертається, і Claude природно тяжіє до того самого пункту. Додатково: тема "місце на диску" була продубльована одночасно в **трьох різних стилях** ("Питання і відповідь" — двічі в одному списку, і "Розбір помилки" — ще раз) — це саме по собі підвищувало ймовірність її вибору.

#### Фікс — два шари одночасно

1. Розширено й де-дубльовано списки прикладових тем у всіх 6 стилів `EXTRA_STYLES` (прибрано потрійне дублювання "місце на диску", додано 3-9 нових пунктів на стиль).
2. Додано `extra_recent_topics` (останні 12 заголовків) у `published.json` — після кожного `extra`-посту перший рядок згенерованого тексту зберігається, і при наступній генерації цей список явно передається в промпт як "не повторюй ці теми". Той самий патерн, що вже роками працює для `content`-постів через `published.published`.

**Перевірено:** build успішний, коміт `adbb0ac`, задеплоєно вручну, homepage 200. Пряму перевірку самого ендпоінту не проведено (немає `AUTOPOST_SECRET` локально) — реальна перевірка станеться на наступному щоденному `extra`-запуску; якщо `extra_recent_topics` коректно з'явиться і наповниться в `published.json` — фікс працює.

#### Оновлений TODO

- [ ] Перевірити за кілька днів, чи `published.json` → `extra_recent_topics` наповнюється і чи повтори тем зникли
- [ ] Слідкувати за відповіддю GitHub Support на звернення про акаунт (тікет #4661396)
- [ ] Пам'ятати деплоїти вручну (`vercel --prod`) після кожного пуша, поки git-зв'язок не відновиться
- [ ] Наступний щотижневий SEO-звіт (cryptolockua-seo-monitor) — перевірити CLS і динаміку індексації
- [ ] Спостерігати за природним виходом запланованих статей (21 серпня 2026 — 2 лютого 2027, 2 пари/тиждень)

---

### Сесія 12 (продовження 47) — Синхронізація репозиторію + запровадження постійного governance/handoff-документообігу

Користувач звернувся вперше після 24-денної перерви (з 29.08 по 22.09.2026). Сесія почалась з повного read-only аудиту (архітектура, автоматизація, SEO, i18n, деплой), потім — окремим кроком — з безпечної синхронізації репозиторію та побудови постійної системи документації для передачі проєкту іншим людям/акаунтам/AI-агентам.

#### Критична знахідка аудиту, одразу виправлена: локальний клон був застарілий на 72 коміти

Перший прохід аудиту дав хибний висновок "автопост-бот не працював 24 дні" — виснований лише з локального `git log`, без `git fetch`. Наступна сесія (та сама розмова) виявила: `origin/main` містив 72 коміти попереду локального `HEAD`, усі — легітимні коміти стану бота (`bot: update published.json [content/middle/extra]`), рівно ~3/день, без жодного пропуску з 30.08 по 22.09.2026. **Висновок про зупинку бота офіційно недійсний** — бот працював безперервно весь час, пишучи напряму в GitHub через API, незалежно від локального клону чи стану Vercel-деплою. Це задокументовано явно (в `PROJECT_STATE.md` → INVALIDATED FINDINGS), щоб жодна майбутня сесія (людська чи AI) не повторила цю саму помилку — і як загальне правило додано в новий `CLAUDE.md`: завжди `git fetch` і звірка з `origin/main` перед будь-яким висновком про стан проєкту.

Реально підтверджено аудитом (не спростовано): `vercel git connect` досі провалюється тим самим способом (6-й задокументований випадок), сайт не мав повного деплою з 29.08 (окремі статті все одно доступні через ISR fallback), `robots.txt` блокує `/_next/` з 24.04.2026 (до червневого краху, тому не першопричина), і в `pages/api/autopost.js` підтверджено код-рівнем race condition (Telegram-відправка до запису стану GitHub, без idempotency).

#### Безпечна синхронізація (виконано покроково, з перевіркою на кожному кроці)

1. Перевірено стан (`git status`, `git fetch`, порівняння HEAD/origin) — 0 попереду / 72 позаду, лінійна історія
2. Проінспектовано всі 11 untracked-файлів — з'ясовано, що більшість (`.artifact-preview-index.html`, `scratch_laptop_keys*.txt`, `scratch_logo*_b64.txt`) стосуються **інших, не CryptoLock проєктів** (зокрема "DATA SAFE SETUP"), випадково опинились у цій робочій папці. `.claude/settings.local.json` — кеш дозволів Claude Code, що охоплює багато різних проєктів одразу; містить одне реальне значення `AUTOPOST_SECRET` у відкритому вигляді (не виведено в чат, не закомічено)
3. Перевірено відсутність колізій між untracked-шляхами і деревом `origin/main` — чисто
4. Створено локальний backup поза репо: `C:\Users\rr\Desktop\cryptolock-local-backup-20260922-2016\`
5. `git pull --ff-only origin main` — чистий fast-forward, HEAD == origin/main (`d9833ec`), жодного merge-коміту

#### Створено постійну систему документації (4 файли)

- **`PROJECT_STATE.md`** (новий) — поточний знімок стану: архітектура, деплой-статус (git push / GitHub / Vercel / production розділені явно), контент-статус, Telegram-статус з явним спростуванням хибної знахідки, відомі баги за пріоритетом, рішення на майбутнє
- **`HANDOFF.md`** (новий) — портативний гайд передачі проєкту іншій людині/акаунту/AI: ієрархія джерел істини, архітектура, деплой-процедура, чек-лист для нового виконавця
- **`CLAUDE.md`** (оновлено, не перезаписано наосліп) — прочитано наявний generic-шаблон повністю, додано специфічний для CryptoLock розділ операційних правил (git-workflow, розділення deploy-станів, формат звіту) **перед** збереженим generic-вмістом, нічого не видалено
- **`DOCUMENTATION.md`** — цей запис, історичні записи не змінювались

#### Оновлений TODO

- [ ] Закомітити лише документаційні файли (`PROJECT_STATE.md`, `HANDOFF.md`, `CLAUDE.md`, `DOCUMENTATION.md`), запушити — деплой НЕ виконувати (це governance-сесія, не продуктова)
- [ ] Свіжий експорт GSC — головна вимога перед будь-якими SEO-змінами (останній наявний чекпоінт — 13.08.2026)
- [ ] Не забувати: `CONTENT.md`, `SETUP.md`, `.claude/`, scratch-файли лишаються локальними, не додані в git автоматично
- [ ] Race condition в `autopost.js` — задокументовано, не виправлено (окрема майбутня задача)
- [ ] `robots.txt` `Disallow: /_next/` — задокументовано, не виправлено (окрема майбутня задача)

---

### Сесія 12 (продовження 48) — Закриття дрібних невідповідностей у governance-документації

Продовження попередньої сесії — чисто документаційне прибирання, без жодних змін у продуктовому коді.

**Зроблено:**
- Виправлено застарілий SHA у `PROJECT_STATE.md` (вказував на коміт до самого себе — оновлено на актуальний)
- З `HANDOFF.md` прибрано посилання на `SETUP.md` (не версіюється, новий виконавець з чистого клону GitHub не повинен натрапляти на інструкцію читати неіснуючий файл) — уся суттєва інформація вже дубльована в самому `HANDOFF.md`
- З `CLAUDE.md` прибрано обидва посилання на `CONTENT.md` (не версіюється, generic-шаблон без CryptoLock-специфіки) — замінено вказівкою дивитись специфічні правила контенту прямо в `DOCUMENTATION.md`, де вони й справді накопичені за сесіями
- У `.gitignore` додано `.claude/` — локальний кеш дозволів Claude Code, що охоплює кілька непов'язаних проєктів і раніше містив відкрите значення `AUTOPOST_SECRET` у кешованому рядку команди. Значення ніколи не комітилось і не виводилось; ротацію секрету свідомо не робив — окреме рішення власника секрету

**Не зроблено (свідомо, поза межами цієї сесії):** ротація `AUTOPOST_SECRET`, видалення `CONTENT.md`/`SETUP.md`/scratch-файлів з диска, будь-який деплой чи зміна production-логіки.

---

### Сесія 12 (продовження 49) — Stage 2D/2E/2F: аудит і виправлення фейкового `/en/{uk-slug}` фолбеку (200 → 308)

Три послідовні read-only аудити (Stage 2D, 2E) і один implementation-етап (Stage 2F), виконані за структурованим протоколом користувача.

**DATE:** 2026-09-22 — 2026-09-23

**PROBLEM:** `/en/{uk-slug}` для UK-статей без реального EN-перекладу повертав живий HTTP 200 з українським контентом (мовчазний фолбек у `lib/posts.js`, задокументований ще липневим фіксом hreflang/canonical, але сам факт 200-відповіді на такому URL ніколи не усувався). Google продовжував сканувати частину цих історичних URL станом на вересень 2026.

**EVIDENCE (Stage 2D, повний код-рівневий аудит):**
- `lib/posts.js:88-94` — фолбек на UK-файл, з прапорцем `isFallback`
- `pages/[slug].js` — `getStaticPaths` не генерує ці шляхи статично, але `fallback: 'blocking'` генерує їх на льоту за запитом
- Canonical коректно вказує на UK-оригінал, hreflang="en" не заявляється (липневий фікс тримається) — але сама сторінка 200, не редирект
- Живо перевірено: 5 із 10 зразкових URL — 200 (жива дублікат-сторінка), 5 — уже точково відредиректжено вручну в `next.config.js`
- 0 входжень цих URL у поточному sitemap, 0 внутрішніх посилань на них у коді/контенті — підтверджено, що Google дізнається про них виключно зі своєї історичної пам'яті, не з живого сайту
- Googlebot UA vs звичайний UA — жодної різниці в поведінці (код не гілкується за User-Agent)

**EVIDENCE (Stage 2E, повна інвентаризація перекладів):**
- 350 UK / 194 EN статей; 140 коректних двосторонніх пар перекладу; 208 UK без жодного перекладу; **2 биті `translatesEn`** (вказують на неіснуючі файли); **1 колізія перекладу** (дві EN-статті claim ту саму UK-статтю, UK.translatesEn обирає лише одну)
- З 82 явних `/en/yak-*` редиректів у `next.config.js`: 53 коректно ведуть на UK, 3 коректно на EN, **щонайменше 15 підтверджено ведуть НЕ туди** (на UK замість реального EN-перекладу, який насправді існує) — включно з точним прикладом із завдання (`yak-vstanovyty-git-windows` мав вести на `/en/how-to-install-git-github-windows`, а вів на UK)
- Висновок: `translatesEn` НЕ досить надійний, щоб автоматична логіка сліпо на нього покладалась

**DECISION:**
1. UK і EN — незалежні редакційні конвеєри за замовчуванням. Немає вимоги, щоб кожна UK-стаття мала EN-відповідник, і навпаки.
2. Автоматична cross-locale підстановка контенту скасовується як клас поведінки: `/en/{slug}` без реального `posts-en/{slug}.md` більше **ніколи** не повертає 200 з чужомовним контентом.
3. **Свідомо НЕ реалізовано** (окрема майбутня задача, не ця сесія): автоматичний вибір EN-цілі через `translatesEn` — визнано ненадійним без окремого фіксу метаданих; 15+ уже підтверджених неправильних явних редиректів у `next.config.js` — не чіпались; ланцюжки редиректів — не сплощувались; `html lang="uk"` на EN-сторінках — не чіпався.

**Зміна коду (`pages/[slug].js`, `getStaticProps`):** якщо `locale === 'en' && post.isFallback === true` — постійний редирект на `/{slug}` (UK-оригінал) замість `props`. `locale: false` в об'єкті редиректу обов'язковий (без нього Next.js домашо­вує `/en/` назад до відносного destination — та сама пастка, що вже задокументована для `next.config.js`). Перевірено живо на локальному production-сервері: результат саме `/slug`, не `/en/slug`.

**Перевірено (7/7 тестів, локальний `next start`):**
| Тест | Результат |
|---|---|
| Колишній fallback 200 (×3 зразки) | 308 → правильний UK URL |
| Наявний явний редирект (`next.config.js`) | Не зачеплено, поведінка та сама |
| Реальна EN-стаття | 200, без змін |
| Реальна UK-стаття | 200, без змін |
| Неіснуючий slug | 404, без змін |
| Перевірка циклів (`curl -L`) | 1 редирект, без циклів |

**EXPECTED RESULT:** нові й повторні запити Googlebot на будь-який ще не відредиректжений вручну `/en/{uk-slug}` тепер отримують 308 замість 200.

**METRIC:** кількість/динаміка сканування історичних `/en/{uk-slug}` URL у категорії GSC "Crawled – currently not indexed" має поступово знижуватись після повторного сканування Google — **не миттєво**, Google сам вирішує темп. **Відновлення трафіку/індексації цією зміною НЕ обіцяється** — це гігієнічний технічний фікс, не SEO-стратегія відновлення сама по собі.

**ROLLBACK:** відкат одного коміту (SHA нижче після пушу).

**FOLLOW-UP (не зроблено цієї сесії):** (1) виправити 15+ підтверджено неправильних явних редиректів у `next.config.js` на реальні EN-цілі, (2) довести до кінця нерозв'язані 6 із 18 випадків Stage 2E через ланцюжок, (3) виправити 2 биті `translatesEn` і 1 колізію перекладу, (4) розглянути автоматичний вибір EN-цілі через перевірений `translatesEn` як окрему майбутню ітерацію, (5) `html lang="uk"` на EN-сторінках, (6) сплощення редирект-ланцюжків.

---

### Сесія 12 (продовження 50) — Stage 2G: деплой і жива валідація фіксу з продовження 49

Деплой коміту `0943b87` (fix cross-locale fallback) на прод + повна жива перевірка, за явним дозволом `DEPLOY: YES`.

**Деплой:** `vercel --prod --yes`, `dpl_FhkpQPEJjsA5pkxxTHYMgJL7tKmB`, статус Ready, 2026-09-23 07:59 EEST, `cryptolockua.com` коректно заалайожено.

**Жива перевірка (8/8 pass):** 3 раніше живих fallback-URL (`yak-pereviryty-versiyu-windows`, `yak-perevesty-windows-v-rezhym-snu`, `yak-nalashtuvanty-group-policy-result`) — тепер 308 → правильний UK URL → 200, без циклів (`curl -L`, по 1 редиректу кожен). Наявний явний редирект (`yak-vstanovyty-git-windows`) — без змін. Реальна EN-стаття, реальна UK-стаття, неіснуючий slug — усі без змін. Googlebot UA vs звичайний — ідентична поведінка. Health-check (головна, `/en/`, sitemap, robots) — усе 200 (окрім очікуваної trailing-slash нормалізації `/en/` → `/en`, не пов'язаної з цим фіксом).

**Моніторинг:** розпочато 2026-09-23 — відстежувати динаміку "Crawled – currently not indexed" для `/en/yak-*` у GSC за наступні тижні, без очікування швидкого результату.
...
### Сесія 12 (продовження 56) — Безпечна діагностика збою обміну токена (CSRF тепер проходить)

**DATE:** 2026-09-24

**PROBLEM:** пізніша реальна спроба TikTok-логіну користувача (та сама сесія, після продовження 55) **пройшла перевірку CSRF-стану успішно** — попередній збій з продовження 54 не повторився, справжня причина того збою лишається непідтвердженою, але він більше не блокує потік. Потік просунувся на крок далі й тепер падає на обміні коду на токен (`POST /v2/oauth/token/`) із загальним повідомленням `Token exchange with TikTok failed.`, без жодних деталей — неможливо було визначити код помилки TikTok, опис чи `log_id` без додаткової діагностики.

**ВАЖЛИВО — супersession:** стан, задокументований комітом `49d5123` (і продовженнями 52-55 вище) — "заблоковано на CSRF" — був коректним ФАКТОМ на момент його запису, але тепер **застарілий**. Нова реальна спроба користувача показала: CSRF ПРОХОДИТЬ, блокер перемістився на обмін токена. Продовження 52-55 залишаються без змін (append-only), цей запис їх не переписує, лише доповнює новішим фактом.

**EVIDENCE:** повідомлено користувачем напряму (два майже ідентичні технічні промпти — "CRYPTOLOCK — DIAGNOSE TIKTOK TOKEN EXCHANGE FAILURE" і "CRYPTOLOCK — TIKTOK TOKEN EXCHANGE DIAGNOSTICS" — об'єднані в одну задачу й один звіт за прямою вказівкою користувача).

**DECISION:** додати діагностику ТІЛЬКИ на кроці обміну токена, ніде більше (не чіпати CSRF-механізм, cookie, `login.js`, scopes, `video.list`, `user.info`, sitemap, SEO, Telegram). Показувати/логувати виключно 4 безпечні поля, які сам TikTok повертає при збої (HTTP статус, `error`, `error_description`, `log_id`) — ніколи `client_key`/`client_secret`/код авторизації/`access_token`/`refresh_token`/значення CSRF-стану чи cookie/повне тіло запиту-відповіді. Не виправляти корінну причину зараз — лише діагностика.

**IMPLEMENTATION:**
- `pages/api/tiktok/callback.js`: новий хелпер `safeStr(value, maxLen=300)` (примусове приведення до короткого рядка або `null`, захист від об'єктів/масивів у полях TikTok). Блок обміну токена перебудовано: парсинг JSON винесено в окремий `try/catch`, щоб відрізнити "неможливо розпарсити (не-JSON відповідь)" від "коректний JSON з помилкою" — на malformed-відповідь рендериться лише "Unexpected TikTok token response" + HTTP статус, без вигаданих полів; на JSON-помилку логуються і рендеряться рівно 4 безпечні поля.
- `lib/tiktokAuth.js`: нова тимчасова `renderTokenDiagnosticPage(diag)` — та сама 502-сторінка збою, але з полями HTTP status / Error / Description / Log ID (або окремим "malformed" виглядом), без жодних реальних секретних значень.
- Успішний шлях (обмін токена, scopes-перевірка, `user.info.basic`, `video.list`, фінальна сторінка результату) — без змін. `pages/api/tiktok/login.js` НЕ чіпався (підтверджено `git diff --stat` — порожньо).

**TESTS:** 26/26 локальних асертів (мокнутий `fetch`) — `invalid_client` (401), `invalid_grant` (400), `invalid_request` (400), malformed/не-JSON відповідь, успішний обмін без змін, скан на відсутність client secret/key/коду авторизації/state у HTML, мережевий збій (`fetch` кидає виняток) — залишається безпечним generic-повідомленням. `npm run build` — успішно.

**RESULT:** діагностика задеплоєна й жива. Продакшн-перевірка виконана безпечним синтетичним запитом — реальний виклик TikTok token endpoint зі свідомо простроченим/фейковим кодом (**не справжній логін**): TikTok повернув HTTP 200 з `error=invalid_grant`, `error_description="Authorization code is expired."`, і реальний `log_id` — нова сторінка коректно показала всі 4 поля. Це підтвердило, що діагностичний шлях живий і правильно парсить реальну форму відповіді TikTok (плаский JSON, не вкладений у `data`; TikTok може повертати HTTP 200 з тілом-помилкою, а не лише 4xx/5xx). **Справжня корінна причина збою обміну токена при реальному (не простроченому, не фейковому) коді авторизації — досі НЕВІДОМА.**

**COMMIT SHA:** `a0110c3`

**DEPLOYMENT:** `vercel --prod --yes`, `dpl_vZeegHeWG7MgAyFXoPw19pWNxhYZ`, `cryptolockua.com`.
...
### Сесія 12 (продовження 71) — Ротація TIKTOK_SCHEDULE_SECRET і редеплой

**DATE:** 2026-09-26

**EVENT:** власник акаунту виконав ротацію `TIKTOK_SCHEDULE_SECRET` у Vercel Production і повідомив, що job у cron-job.org налаштований, але реальна автентифікація планувальника ще не підтверджена. Завдання на редеплой НЕ вказало причину ротації, тому причину тут НЕ записано і не вгадується.

**DECISION:** оскільки serverless-функції не перечитують env vars без нового деплою, потрібен редеплой поточного коду. Змін коду немає.

**IMPLEMENTATION:** `vercel --prod --yes` з незміненої версії `0670579` → `dpl_AEfwg85byMR77KuMidkJiFVMZdqk`, READY, аліас `cryptolockua.com` (і `www`).

**SECURITY:** значення `TIKTOK_SCHEDULE_SECRET` не читалось, не використовувалось, не виводилось і не зберігається у Git/документації; перевірки використовували лише свідомо неправильний Bearer.

**RESULT:** безпечні перевірки після редеплою: `GET /api/tiktok/collect` → 405, `POST` без `Authorization` → 401, `POST` з явно невірним Bearer → 401, `/tiktok-connect` → 200. Вони доводять лише, що endpoint живий і застосовує auth (401, а не 500, також свідчить, що секрети налаштовані й не однакові) — вони НЕ доводять, що новий schedule-секрет працює. Автентифікований виклик collector'а НЕ виконувався, снапшот НЕ створювався й НЕ оновлювався, Redis НЕ чіпався.

**STATUS:** cron-job.org job налаштований — **за словами власника акаунту** (дашборд cron-job.org з боку Claude не перевірявся). Реальна автентифікація планувальника зі `TIKTOK_SCHEDULE_SECRET` — **NOT YET VERIFIED**. Автоматичне заплановане виконання — **NOT YET VERIFIED**. Analytics writer, manual-режим і token refresh лишаються VERIFIED (продовження 62-69).

**COMMIT SHA:** немає (без змін коду — лише ротація env var + редеплой; production-версія та сама, що й у продовженні 70: код `4f0f109`, HEAD `0670579`).
...
### Сесія 12 (продовження 73) — GSC Indexing Backlog Classification Audit: верифікований аудит беклогу непроіндексованих URL

**DATE:** 2026-09-26

**PROBLEM:** у Google Search Console велика кількість URL має статуси «Crawled — currently not indexed» і «Discovered — currently not indexed». Останній зафіксований підсумок «проіндексовано» — 13.08.2026 (66 / 792 / 858). До будь-якого SEO-рішення треба було встановити, з чого фактично складається цей беклог: скільки в ньому реальних поточних сторінок, придатних до індексації за правилами сайту, а скільки — застарілого/недієвого шуму (редіректи, історичні `/en/`-fallback, теги), і чи видно з цього діючу причину. Завдання було **лише на читання** й документування: жодних змін коду, SEO, sitemap, редіректів, robots, контенту, GSC чи експериментів.

**EVIDENCE (VERIFIED — read-only):**

*Метод.* Два експорти GSC Coverage Drilldown (Crawled — 650 рядків; Discovered — 178 рядків) розібрано без сторонніх залежностей; кожен URL класифіковано проти **поточного** репозиторію (posts/, posts-en/, `next.config.js` з 370 правилами редіректів, `pages/sitemap.xml.js`, `pages/[slug].js`, `pages/tags/[tag].js`) з емуляцією правил зіставлення Next.js для редіректів (`locale:false`, percent-encoded джерела, дефолтна локаль). Усі 828 рядків потім звірено з живими HEAD-відповідями production (`redirect: manual`): усі збіглися з прогнозом; 4 транзієнтні мережеві помилки перевірено повторно — збіглися; усі 208 унікальних цілей редіректів/fallback повертають 200. Під час звірки з живими відповідями було виявлено й виправлено три прогалини власного класифікатора (зіставлення `/uk/`-префікса для `locale:false`, percent-encoded джерела, дволанцюгові trailing-slash редіректи) — наведені нижче числа вже скориговані. Дані експортів закінчуються **2026-09-21**; класифікація відображає production станом на 2026-09-26.

*Підсумок.* **828** URL: **650** Crawled + **178** Discovered; жоден URL не в обох групах. Реальних поточних URL, придатних до індексації за правилами сайту, — **462** (441 стаття = **290 UK** + **151 EN**; **13** інструментів; **8** інших сторінок).

*Crawled (650):* **284** реальних поточних URL, придатних до індексації за правилами сайту (209 UK-статей, 55 EN-статей, 13 інструментів, 7 інших сторінок) і **366** застарілих/недієвих: **106** історичних EN-fallback (`/en/{uk-slug}` без EN-файлу; тепер 308→UK), **75** URL тегів (72 існуючі noindex-теги, 2 індексні сторінки тегів, 1 тег, що тепер дає 404), **183** редіректи (179 явних правил — 117 EN-префіксованих і 62 UK — плюс 4 нормалізації trailing slash; усі ведуть на живу сторінку 200), **2** неіснуючі `/en/…`. 8 логічних дублікатів через trailing slash. Усі 73 «неіндексні» теги востаннє скановано **до** коміту `247a24a` (2026-08-12, noindex + вилучення з sitemap). Лише 55 із 269 EN-префіксованих URL «у формі статті» — справжні EN-статті; ще 39 рядків `/en/…` — явні редіректи на існуючі UK-сторінки. Діапазон дат останнього сканування: 2026-04-30 … 2026-09-21 (медіана 2026-07-17).

*Discovered (178):* **усі 178** — реальні поточні URL, придатні до індексації за правилами сайту (81 UK-стаття, 96 EN-статей, 1 інша сторінка `/bookmarks`), **усі в sitemap**, усі мають плейсхолдер «ніколи не скановано» `1970-01-01` (сире значення експорту `25569`). Застарілого шуму в цій групі немає.

*Свіжість (асоціація, не доведена причина).* **104 із 112 (93%)** статей, опублікованих 2026-06-01 або пізніше, є в Discovered (UK 52 із 57: 4 Crawled / 52 Discovered / 1 в жодній групі; EN 52 із 55: 1 / 52 / 2). Серед статей, опублікованих раніше, у Discovered лише 73 із 358 (20%). Медіана дати публікації: UK Discovered 2026-06-25 проти UK Crawled 2026-03-26. 29 опублікованих статей (23 UK + 6 EN) не в жодній із двох груп — їхній статус за цими експортами невідомий (можуть бути проіндексовані або мати інший статус).

*Sitemap.* Sitemap, виведений із коду репозиторію, **точно збігається з живим**: **483** URL (13 статичних + 313 UK + 157 EN). 453 з 483 URL sitemap належать до однієї з двох груп, 30 — ні. Кожна стаття з обох груп є в sitemap.

*Дрібні знахідки (hygiene):* (1) єдиний URL sitemap, що редіректить, — `/yak-zashyfruvaty-dysk-bitlocker`: справжня опублікована UK-стаття, «затінена» явним правилом на `/yak-uvimknuty-bitlocker-windows-11`; водночас вона в sitemap і в його high-priority списку; (2) літеральний `/search?q={search_term_string}` було скановано як URL — його породжує JSON-LD WebSite `SearchAction` (`components/Layout.js:192`, `pages/index.js:70`); (3) mojibake-URL `/en/yak-nalashtuvanty-dvokrok…` не покритий правилами редіректів (вони обробляють лише варіант із `/uk/`); (4) 5 наявних сторінок інструментів відсутні в sitemap: `base64`, `hash`, `ip-info`, `port-checker`, `regex`.

*Лінкування та розмір (описово).* Внутрішнє лінкування в тілі статей розріджене: медіана вхідних body-посилань — 0, приблизно 73–84% статей у групах не мають жодного; **групи мало відрізняються за цим показником** (UK Crawled 74% / UK Discovered 77% / EN Crawled 75% / EN Discovered 84% без вхідних). Обсяг прози (без fenced-коду, наближено): медіани корпусу UK 248 / EN 315 слів; Discovered UK 162 проти Crawled UK 263; Discovered EN 275 проти Crawled EN 387.

*Однаковий slug.* Жоден slug не існує одночасно в `posts/` і `posts-en/`; усі 142 посилання `translatesEn` ведуть на **інший** EN-slug. 159 slug є в GSC і як `/slug`, і як `/en/slug`: у 132 випадках реальна лише UK-сторінка, обидві реальні — 0 статей (+2 статичні сторінки), реальна лише EN — 0, у 25 випадках жоден варіант не віддає 200. Із 103 перекладних пар з обома опублікованими сторонами: 57 — обидві Discovered, 13 — обидві Crawled, 21 — UK Crawled / EN Discovered, 6 — навпаки, 6 — одна зі сторін у жодній групі.

*Перетин з Crawl Priority Experiment.* Усі 10 URL експерименту (5 TEST + 5 CONTROL) — у **Discovered**, жоден не в Crawled. Це валідна **передінтервенційна** базова лінія, що збігається з базою, зафіксованою на старті експерименту.

*Обмеження.* (a) Експорти закінчуються 2026-09-21, тобто **до** виправлення fallback (задеплоєно 2026-09-23, коміт `0943b87`) і **до** старту Crawl Priority Experiment (2026-09-24) — цей експорт **не вимірює ефекту експерименту** і не показує ефекту виправлення fallback; (b) експорти містять лише ці два статуси — загальної кількості «проіндексовано» немає; (c) для Discovered дата сканування — плейсхолдер, тому «давність» цієї групи не вимірювана; (d) графік Discovered у GSC волатильний (0 → 210 → 0 → 219 → 161 → 178 між 2026-07-11 і 2026-09-21), тому окремі денні значення слід трактувати обережно; (e) підрахунки слів наближені; (f) проміжний CSV з класифікацією 828 рядків був робочим артефактом сесії й у репозиторій не збережений — на 2026-10-08 класифікацію треба повторити на свіжому експорті.

**DIAGNOSIS:**
- **ФАКТИ (перевірені):** беклог — це реальна поточна проблема сканування/індексації, а не лише історичний шум: 462 реальні поточні URL, придатні до індексації за правилами сайту (із них 441 стаття), стоять у «не проіндексовано»; водночас 366 із 650 рядків Crawled — застарілі/недієві. Discovered-група складається виключно з реальних поточних URL, усі в sitemap, і 93% свіжих (від 2026-06-01) статей належать саме до неї.
- **ГІПОТЕЗИ (НЕ перевірені, НЕ підвищуються до фактів):** (1) Google може виділяти мало уваги сканування новим URL; (2) застарілі URL (редіректи, fallback, теги) можуть споживати увагу сканування, яку інакше отримали б нові; (3) слабке внутрішнє лінкування може сприяти проблемі. Розріджений граф body-посилань **ще не доводить**, що внутрішнє лінкування є причиною: групи Crawled і Discovered за цим показником схожі.
- **НЕ ВСТАНОВЛЕНО:** причину обвалу трафіку 13 червня 2026 аудит **не доводить і не спростовує** — це окреме, досі нерозв'язане питання (Stage 2C у цьому документі). Він також не встановлює, що дрібні hygiene-знахідки впливають на беклог.

**DECISION:** **жодних нових змін crawl/indexing SEO до 2026-10-08, якщо не з'явиться P0-проблема production/індексації.** Причина: зберегти валідність чинного Crawl Priority Internal-Link Experiment і не змішувати втручання, щоб будь-який рух TEST відносно CONTROL можна було приписати саме експерименту. Чотири hygiene-знахідки лишаються P2 (inspect-only до 2026-10-08) і **не** перекласифіковуються в P0 без доказів. Жодних змін коду, SEO, sitemap, редіректів, robots, контенту, експериментів, production, Vercel, GSC, GA4, Telegram чи TikTok цим аудитом не внесено; URL-подань чи запитів на індексацію не було.

**EXPECTED RESULT:** після завершення вікна експерименту (2026-10-08) ефект внутрішніх посилань можна оцінити чисто (TEST проти CONTROL, без змішування з іншими втручаннями), а на свіжому експорті — побачити, чи рухається ширший беклог (зокрема чи зменшилась кількість застарілих рядків після виправлення fallback 2026-09-23).

**METRIC:** (1) критерій успіху експерименту без змін — ≥3 із 5 TEST URL переходять із Discovered у Crawled-not-indexed або Indexed порівняно з рухом CONTROL за те саме вікно; (2) рух 462 поточних URL, придатних до індексації, за статусами між експортами; (3) частка застарілих рядків (redirect / fallback / tag) у Crawled до і після 2026-09-23; (4) свіжий загальний показник «проіндексовано».

**RESULT:** аудит завершено й **VERIFIED**; його факти, обмеження та гіпотези зафіксовано в `PROJECT_STATE.md` (GSC STATUS, KNOWN BUGS, DECISIONS #9, ACTIVE EXPERIMENTS, COMPLETED WORK, BACKLOG, NEXT ACTION). Crawl Priority Experiment — без змін, RUNNING; Telegram Poll Experiment — без змін. Причина обвалу 13 червня — **не з'ясована**. `HANDOFF.md` не змінювався (нових довготривалих архітектурних/операційних фактів немає).

**FOLLOW-UP:**
- **P1:** оцінити Crawl Priority Experiment 2026-10-08 на свіжих експортах Crawled + Discovered (TEST проти CONTROL); після цього дослідити 462 поточні URL беклогу, придатні до індексації; переглянути внутрішнє лінкування / контентну архітектуру, якщо TEST не випередить CONTROL.
- **Наступна дія:** дочекатися завершення експерименту 2026-10-08, отримати свіжі експорти Crawled + Discovered, порівняти TEST і CONTROL, оглянути рух сканування й вирішити — масштабувати механізм crawl-priority чи тестувати сильнішу архітектуру внутрішнього лінкування/контенту. Це **не** «виправляти SEO зараз».
- **P2 / hygiene (не реалізовувати до завершення вікна):** оглянути «затінену» статтю `/yak-zashyfruvaty-dysk-bitlocker`; виправити літеральне сканування `/search?q={search_term_string}`; переглянути прогалини охоплення правил редіректів; переглянути 5 наявних інструментів, відсутніх у sitemap.

**SECURITY:** документ містить лише публічні URL-шляхи та агреговані лічильники; жодних токенів, секретів чи облікових даних.

**COMMIT SHA:** змін коду немає (docs-only); docs-коміт — див. `git log` (повідомлення `docs: record verified GSC indexing backlog audit`). Build/deploy не запускались.

---

### Сесія 12 (продовження 74) — P0-стримування: призупинення AuditShield-просування в Telegram autopost; прибрання платіжних реквізитів із публічного HEAD

**DATE:** 2026-09-26

**PROBLEM:** верифікований read-only аудит продукту/безпеки AuditShield (2026-09-26) показав, що автоматичне просування продукту з каналу @cryptolock888 не можна продовжувати: підтверджених реальних покупок 0, підтвердженого реального доходу $0, платний шлях (перевірка ліцензії → отримання модуля → запуск) жодного разу не проходив наскрізно в production після proxy-перебудови 2026-09-23, а в самому боті є P0-проблеми безпеки/надійності (адмін-команди й адмін-callback'и без перевірки адміна, неавтентифікований `/launch`). Просування трафіку в такий бот — це пряме спрямування людей в уразливість. Водночас Telegram Poll Experiment активний до 2026-10-08 і його треба зберегти. Окремо аудит виявив, що в ПУБЛІЧНОМУ репозиторії CryptoLock у документації лежать платіжні реквізити AuditShield (категорії: номер платіжної картки та адреса криптогаманця).

**EVIDENCE:**
- `pages/api/autopost.js`, `type=middle`: за парністю дня місяця UTC — **парні дні** = `middle-engage` (A/B експеримент poll ↔ text через `engage_ab_index`), **непарні дні** = AuditShield-промо (`PROMO_MODULES` + `promptPromo`, що містив назву продукту, ціни, `@`-згадку бота та заклик спробувати демо). Окремий `type=promo` (та сама реклама) мав бути неактивним за документацією (cron «Promo Post 11:30» — «залишено неактивним»), але код лишався досяжним із секретом.
- Стан на момент задачі (лише читання `published.json`): `pending: null`, `lock: null`, `promo_index=75`, `engage_ab_index=2`, `engage_index=66`, `extra_index=93`. Логіка orphaned-pending у handler'і ніколи не повторює відправку, тож уже згенерованого, але не відправленого промо після деплою випливти не могло.
- Пошук по всіх відстежуваних файлах (за формою значення, без виводу самих значень): платіжні реквізити були в **одному** рядку `DOCUMENTATION.md` (таблиця «Тарифи продажу клієнтам»), не в runtime-коді. IBAN-подібних значень немає. `.claude/` git-ignored.

**DECISION:** **PAUSE PROMOTION, NOT SUNSET.** Продукт не закривається й не схвалюється — просування зупиняється до P0-hardening'у.
1. Непарний `middle`-слот **зберігає** розклад «3 пости/день» (content / middle / extra) і час, але замість реклами публікує **самостійний практичний Windows how-to** із того самого списку з 22 тем. У промпт потрапляють лише `module` і `when`; жодної назви продукту, `@`-згадки бота, цін, оплати, закликів купити/спробувати демо/ліцензію, посилань. `PRODUCT_BASE` і `promptPromo` видалено.
2. **`promo_index` збережено як лічильник ротації тем** (без нового persistent state); ідентифікатор `standalone-topic:<тема>`; commit-мітка бота змінена з `[middle-promo]` на `[middle-standalone]`, щоб момент перемикання був видимий в історії git під час оцінки 2026-10-08.
3. `type=promo` тепер повертає `{ok:true, skipped:true, reason:'promo_paused'}` **до** будь-яких викликів GitHub/Anthropic/Telegram (інакше невідомий `type` потрапив би в гілку `content` за замовчуванням і просунув би лічильники статей).
4. **Парні дні — без змін:** `engage_ab_index`, `poll_recent_topics`, `engage_index`, чергування poll → text → poll → text, lock/pending/finalize. `engage_ab_index` НЕ просувається на непарних днях. `published.json` вручну не редагувався.
5. Платіжні реквізити прибрано з поточного HEAD публічного репозиторію (один рядок `DOCUMENTATION.md`; замінено формулюванням, що реквізити оплати операційні/приватні й не мають зберігатися в публічному репозиторії). Це **єдине** виключення з правила «історичний документ — лише append»: змінено рівно один рядок старого запису, щоб прибрати чутливі значення; решту запису не чіпано. **Історію git НЕ переписано, нічого НЕ ротовано** — значення лишаються в старих комітах (рішення за власником).
6. Ні AuditShield-репозиторії, ні Railway, ні AuditShield-бот, ліцензії, CRM, SEO-експеримент, TikTok, черга статей, розклад content/extra не змінювались.

**EXPECTED RESULT:** з першого непарного дня (2026-09-27) канал більше не публікує AuditShield-рекламу, а непарний `middle`-пост — корисна самостійна порада; розклад і парність Poll Experiment збережені.

**METRIC:** (1) наступний бот-коміт `published.json` за непарний день має мітку `[middle-standalone]`, `promo_index` +1, `engage_ab_index` не змінився; (2) текст опублікованого поста не містить згадки продукту/бота/цін/CTA; (3) на парні дні поведінка й лічильники — як раніше; (4) публічний HEAD не містить платіжних значень (повторний скан за формою).

**RESULT:**
- **Тести (локально, повністю змокано — жодного реального мережевого виклику й жодної відправки в Telegram):** диференціальний тест «оригінал (git HEAD) проти нового коду», **40/40**: транскрипти парного дня (промпти, payload'и Telegram, записи стану) **побайтово збігаються** з оригіналом як для poll-, так і для text-ходу, у dry-run і в повному циклі; `engage_ab_index` 2→3 та 3→4 як і раніше; 8-денна послідовність із персистентним станом дає ту саму парність і те саме чергування; непарний день — один пост, промпт без продукту, `engage_ab_index`/`engage_index`/`extra_index`/`count`/`poll_recent_topics` не змінені, `promo_index` 75→76, послідовність lock → pending → send → finalize така сама; усі 22 теми × 2 ротації дають промпт без згадок продукту (44/44); `type=promo` — 0 зовнішніх викликів; `content`/`extra`/`engage` — ідентично оригіналу. Тест також підтвердив, що вихідна версія справді містила продукт у промпті (тест здатен це виявити).
- `git diff --check` — чисто; `npm run build` — успішно; скан доданих рядків — жодних значень картки/гаманця чи токенів; повторний скан усіх відстежуваних файлів — 0 збігів за формою картки/гаманця.
- **Код:** коміт `3559983` («fix: pause AuditShield promotion pending hardening»), запушено в `main` (fast-forward).
- **Деплой:** `vercel --prod --yes` → `dpl_34iYhpK32qjzuSJL2gG5G1TKaeME` (`crypto-lock-1js9cwp3s-crypto-lock.vercel.app`), статус Ready, target production, alias `cryptolockua.com` + `www.cryptolockua.com`. Попередній production-деплой: `crypto-lock-az1g674v0` / `dpl_AEfwg85byMR77KuMidkJiFVMZdqk` (коміт-джерело `0670579`). Деплой виконано з чистого відстежуваного дерева, що дорівнює `3559983`.
- **Production-перевірка (без секрету):** головна 200; `/api/autopost` без секрету → 401 (handler завантажується й захищений). Автентифіковану поведінку в production я не викликав (потрібен `AUTOPOST_SECRET`, а будь-який виклик міг би відправити реальний пост) — її підтверджує локальний тест на тому самому коді та перший живий непарний цикл 2026-09-27 (METRIC вище).
- Реальних відправок у Telegram під час тестів: **жодної**.

**FOLLOW-UP:**
- Перевірити перший живий непарний цикл 2026-09-27 (METRIC 1–2).
- **Confounder Poll Experiment (зафіксовано в PROJECT_STATE.md → ACTIVE EXPERIMENTS):** з 2026-09-26 непарні `middle`-пости змінились з AuditShield-реклами на самостійні how-to. Реалізація й дата рішення експерименту (2026-10-08) не змінені, але канал більше не «ідеально ізольований»: підсумкова оцінка 2026-10-08 **мусить** згадати цю зміну середовища й не відносити зрушення канальних метрик (динаміка підписників, загальна частка реакцій) лише до втручання poll/text.
- **Наступна дія — окремий набір змін:** AuditShield P0-hardening (адмін-авторизація на всі команди/callback'и, автентифікація або видалення `/launch`, наскрізна перевірка платного шляху), далі owner E2E-тест на чистому Windows ПК і вимірюваний період валідації. Перехід PAUSE → resume/KEEP або SUNSET — лише після цього.
- Рішення власника: що робити з платіжними реквізитами, які лишаються в історії git цього публічного репозиторію.
- Не зроблено цим кроком (окремі задачі): виправлення тверджень на `/tools/auditshield`, оновлення застарілого giveaway-пакета, будь-які зміни самого AuditShield.

**ROLLBACK:** повний відкат коду — `git revert 3559983` + повторний деплой (**повернув би AuditShield-рекламу й платіжний рядок у HEAD — робити лише свідомо**); швидкий відкат production без нового коміту — promote попереднього деплою `crypto-lock-az1g674v0`. `published.json` (стан бота) не відкочувати.

**SECURITY:** запис не містить значень платіжних реквізитів, токенів чи секретів.

**COMMIT SHA:** код — `3559983`; docs-коміт цього запису — див. `git log` (`docs: record AuditShield promotion pause ...`).

---

### Сесія 12 (продовження 75) — Синхронізація закриття P0-фази безпеки AuditShield

**DATE:** 2026-09-26

**PROBLEM:** запис продовження 74 зафіксував паузу просування AuditShield і наступну дію «AuditShield P0-hardening у репозиторіях AuditShield». Ця дія виконана й верифікована; журнал рішень і `PROJECT_STATE.md` мають відображати поточний стан, щоб у них не лишалось застарілих тверджень на кшталт «P0 не виправлено».

**EVIDENCE** (детальний запис — `security-audit-private/DOCUMENTATION.md`, сесія 4, коміт `ae6c4e7`; вміст логів і значення токенів тут не наводяться):
- `audit-shield-bot` `57cdb58` — адмін-команди та callback'и fail-closed; `/launch` автентифікований коротким підписаним квитком.
- `audit-shield-bot` `51cbb1b` — витік токена Telegram у логи Railway виправлено (логер `httpx` на WARNING); токен **ротовано власником** (старий відкликано, новий встановлено в Railway, сервіс передеплоєно, `/start` працює); значення ніде не документуються.
- `audit-shield-bot` `d63c0ad` — виправлено падіння `/start` для адміна.
- Поточний деплой AuditShield на Railway: `93a49a78-edd5-4325-ac55-bf247b698f6e`, SUCCESS; 56/56 тестів; production-перевірка: `/health` 200, `/launch` без авторизації → 401 і стан не змінено; підрахунок рядків у логах — 0 токеноподібних рядків.
- Верифікація власником після деплою: **4/4** (адмін `/start`; «До меню»; «Панель адмiна»; `/start` не-адміна без кнопки адміна).

**DECISION:** рішення №10 без змін — **PAUSE PROMOTION, NOT SUNSET**. P0-фазу безпеки AuditShield **закрито**. Наступна дія — **AuditShield P1 licensing/reliability** (головне: запуск ліцензії не має безповоротно списуватись до успішної видачі модуля). Просування **не відновлювати** до завершення P1, owner E2E-тесту на чистому Windows ПК і вимірюваного періоду валідації. SEO-експеримент і Telegram Poll Experiment цим записом **не змінюються**.

**Історична корекція:** падіння `/start` для адміна **не було спричинене P0-hardening'ом**. Це існуюча production-помилка: `kb.inline_keyboard.append(...)` з'явився 2026-05-11 за вже закріпленої `python-telegram-bot==20.7` (у ній `inline_keyboard` — незмінний tuple); hardening не змінював `start()` і код меню. Виявлено під час верифікації власником після ротації токена (адмін до того користувався `/admin`).

**RESULT:** `PROJECT_STATE.md` синхронізовано: AUDITSHIELD STATUS (PROMOTION: PAUSED; SECURITY: P0 FIXED / VERIFIED; токен: ROTATED, значення не документується; реальних покупок 0, доходу $0; продукт не sunset), KNOWN BUGS (застарілі P0-пункти прибрано, відкриті P1/P2 збережено), COMPLETED WORK, BACKLOG, NEXT ACTION, DECISIONS #10, CURRENT KPIs. Docs-only: жодних змін коду, збірки, деплою, Telegram-відправок, SEO чи експериментів у цьому репозиторії.

**FOLLOW-UP:** AuditShield P1 (`runs`/`runs_max`, `runs_used`, `converted`, атрибуція джерела, вичерпання квоти GitHub через `/verify-license`); owner E2E-тест; вимірюваний період валідації; оновлення застарілого giveaway-пакета; виправлення тверджень лендингу `/tools/auditshield`; рішення власника щодо платіжних реквізитів, що лишаються в історії git цього публічного репозиторію (з HEAD прибрано в продовженні 74).

**SECURITY:** запис не містить токенів, значення `ADMIN_CHAT_ID`, платіжних реквізитів, даних клієнтів, ключів чи паролів ліцензій.

---

### Сесія 12 (продовження 76) — Синхронізація: основний P1 AuditShield (списання запуску при видачі модуля) виправлено

**DATE:** 2026-09-26

**PROBLEM:** після закриття P0-фази (продовження 75) головним відкритим P1 AuditShield було те, що платний запуск списувався в `/verify-license` ще ДО видачі модуля, тож збій після верифікації (помилка отримання ядра, втрата відповіді, таймаут, рестарт) спалював запуск без видачі. Журнал рішень і `PROJECT_STATE.md` мають відображати, що це виправлено, і що саме лишилось недоведеним.

**EVIDENCE** (детальний запис — `security-audit-private/DOCUMENTATION.md`, сесія 5, коміт `9be7552`):
- `audit-shield-bot` `1f52d82`: запуск списується при видачі модуля однією умовною CAS-записом разом із квитанцією ідемпотентності (хеш випадкового `jti` квитка, зберігається в `licenses.json`); `/verify-license` лише автентифікує; `/get-module` спочатку отримує ядро; повтор того самого квитка не списує вдруге (також після рестарту); CAS-конфлікти й таймаути — через перечитування стану, без повторів застарілого `sha`; лаунчер: verify → get-module (обмежений повтор тим самим квитком) → best-effort `/launch`.
- Тести: 93/93 (37 нових; 35 із них падають на попередньому коді); справжній перезапуск процесу; справжній `launcher.ps1` у PowerShell проти ЛОКАЛЬНОГО mock-сервера.
- Railway: `3fa8eb20-f4f0-4b5c-872c-0589c35c3cbb`, SUCCESS (попередня `93a49a78-…`). Production-перевірки лише НЕспоживчі (health, відмови без авторизації/з недійсним токеном, приватний репозиторій стану не змінився); реальну ліцензію не використовували.

**DECISION:** рішення №10 без змін — **PAUSE PROMOTION, NOT SUNSET**; продукт **не** позначено KEEP/validated. Виправлення доведено кодом, тестами й деплоєм, але **не реальною ліцензією**. Наступна дія — **контрольований owner E2E на чистому Windows ПК** з окремою тестовою ліцензією; потім решта P1 (CRM `runs`/`runs_max`, синхронізація `runs_used`, `converted`, атрибуція джерела, квота GitHub у `/verify-license`). Просування **не відновлювати**.

**RESULT:** `PROJECT_STATE.md` синхронізовано (AUDITSHIELD STATUS, KNOWN BUGS, COMPLETED WORK, BACKLOG, NEXT ACTION, DECISIONS #10, CURRENT KPIs). Docs-only: жодних змін коду, збірки, деплою, Telegram-відправок, SEO чи експериментів у цьому репозиторії. SEO-експеримент і Telegram Poll Experiment не змінювались.

**FOLLOW-UP:** owner E2E; решта AuditShield P1; вимірюваний період валідації; оновлення giveaway-пакета; виправлення тверджень лендингу `/tools/auditshield`; рішення власника щодо платіжних реквізитів в історії git цього репозиторію. Залишкові ризики (див. сесію 5 приватного репозиторію): вузький випадок, коли списання застосувалось, а всі 3 повтори лаунчера теж не дійшли; адмін-видача ліцензій досі використовує старий запис із повтором `sha`.

**SECURITY:** запис не містить токенів, значення `ADMIN_CHAT_ID`, платіжних реквізитів, даних клієнтів, ключів чи паролів ліцензій, сирих `jti`/квитків.

---

### Сесія 12 (продовження 77) — Синхронізація: результат owner E2E AuditShield і усунення збоїв ядра аудиту

**DATE:** 2026-09-26

**PROBLEM:** продовження 76 зафіксувало, що основний P1 AuditShield (списання запуску при видачі модуля) виправлений, а наступна дія — контрольований owner E2E на тестовій ліцензії. Цей E2E відбувся того ж дня. Він підтвердив шлях платної видачі, але **рушій аудиту (`audit_core.ps1`) не пройшов приймання**: дев'ять модулів завершились помилкою виконання на Windows PowerShell 5.1, власник вимушено натискав Ctrl+C під час «Installed Software» (~4 хв 01 с), Defender показано як вимкнений при увімкненому, а розділ міг показати «нічого не виявлено» після збою модуля. Поки це не було задокументовано, стан проєкту («очікує owner E2E») був застарілим.

**EVIDENCE** (детальний запис — `security-audit-private/DOCUMENTATION.md`, сесія 6, коміти `503fb94` і `d065121`):
- **PASS:** `/verify-license`; видача модуля; списано рівно один запуск (тестова ліцензія `OWNER_E2E_2026-09-26`, 1 із 3; це тест власника, доходу немає); квитанція збережена; launch-телеметрія; звіт згенеровано й відкрито; вікон Defender/SmartScreen не спостерігали.
- **FAIL:** помилки виконання — Running Processes і Autorun (невалідні regex), Network Activity (присвоєння `$PID`), DNS/Services/Scheduled Tasks/Login Events/Firewall/Executables History (`(If …)` розбирається як команда в PS 5.1); повільні етапи; хибний Defender (декодування `productState` `0x061100`); чисте «не виявлено» після збою (Firewall); консоль завжди писала AUDIT COMPLETE. Ті ж помилки були й у травневих логах. Причина, чому тести не помітили: у ядрі не було жодного тесту, а 93 тести бота покривають лише бот/ліцензування.
- **Виміряно** (не вгадано): Installed Software = 13 однакових WMI-запитів по ~21 с (99,6 % з ~260 с); Firewall — per-rule запити (~90 % з 45 с); Document History — 250 обходів, кожен 14–16 с на ПК власника; Running Processes раніше не досягався (падав миттєво), а його реальна вартість — 134 с, з них 96 % перевірка підписів (94 виклики на 40 різних файлів). Розбіжність 140 с (E2E) і 14–16 с на обхід тут **не пояснена**.
- **Виправлення** (лише `security-audit-private`; код бота, ліцензування, `/verify-license`, `/get-module`, `/launch`, авторизація, логування токенів, CRM, платежі — не змінювались): усі дев'ять помилок; статус кожного модуля (SUCCESS/ERROR/SKIPPED, час, очищений опис); розділ невдалого модуля позначається неповним і не може показати зелене «нічого не знайдено»; AUDIT COMPLETE або AUDIT PARTIAL — N MODULE ERRORS; декодування `productState` за бітами з авторитетним `Get-MpComputerStatus`; один WMI-запит Office (ліміт 90 с), bulk-запит портів Firewall, один обхід документів `-Include` (ліміт 300 с), кеш перевірки підписів за шляхом.
- **Тести:** `tests/Run-AuditCoreTests.ps1` на справжньому PowerShell 5.1, 49 тестів: **старе ядро 6 пройшло / 41 впало** (кожна помилка E2E відтворена дослівно, зокрема «DISABLED» для увімкненого Defender на реальному ПК), **нове ядро 49/49**. Повний прогін усіх 22 модулів на ПК власника (без підвищення, поза ліцензією/ботом): усі SUCCESS, AUDIT COMPLETE, 335 с.
- **Деплой:** push `34cba48..503fb94` у `main` приватного репозиторію; бот віддає ядро звідти, тому окремий деплой бота не потрібен і не виконувався. **Жоден запуск тестової ліцензії не витрачено** (залишилось 2).

**DECISION:** рішення №10 без змін — **PAUSE PROMOTION, NOT SUNSET**. Статус: **PAID DELIVERY VERIFIED / AUDIT CORE REMEDIATION IN PROGRESS**; AuditShield **не** позначено VERIFIED IN PRODUCTION і не KEEP/validated. Ризик-бал не переробляли: це проста сума балів подій, **а не шкала 0–100**; бали різних запусків непорівнянні (модулі, що раніше падали, тепер дають знахідки: 7490 у E2E проти 6455 у виправленому прогоні на тому ж ПК).

**RESULT:** `PROJECT_STATE.md` синхронізовано (AUDITSHIELD STATUS, KNOWN BUGS, COMPLETED WORK, NEXT ACTION). Docs-only: жодних змін коду, збірки, деплою, Telegram-відправок, SEO чи експериментів у цьому репозиторії; експерименти (SEO Crawl Priority, Telegram Poll) без змін.

**FOLLOW-UP:** **другий owner E2E виправленого рушія — лише після схвалення власником**, з підвищенням (реальний шлях лаунчера) на тій самій тестовій ліцензії; перевірити SUCCESS усіх 22 модулів, AUDIT COMPLETE, Document History в межах ліміту, коректний Defender, відсутність хибного KMS-попередження (детектор активаторів у System Info раніше не працював на 5.1 і тепер живий), відсутність потреби в Ctrl+C. Відкрито, не змінювалось: «File System Activity» 106,7 с (22 с в E2E) і «Data Transfer Indicators» 69,7 с не діагностовані, модулі не мають власного ліміту часу; `audit_demo.ps1` у репозиторії бота містить той самий дефектний код; порожні `Catch {}`; решта AuditShield P1 (CRM `runs`/`runs_max`, `runs_used`, `converted`, атрибуція, квота GitHub у `/verify-license`).

**SECURITY:** запис не містить токенів, значення `ADMIN_CHAT_ID`, платіжних реквізитів, даних клієнтів, ключів чи паролів ліцензій, сирих `jti`/квитків; у ньому немає зібраних даних із машини власника.

---

### Сесія 12 (продовження 78) — Синхронізація: AuditShield REPORT V2 реалізовано (очікує owner E2E)

**DATE:** 2026-09-26

**PROBLEM:** продовження 77 зафіксувало, що збої ядра аудиту виправлено, але звіт v1 лишався головною слабкою ланкою продукту: сума балів без межі як головний результат (≈7490 в E2E), висновки «піратство» з назви каналу KMS/Volume, передача даних за підрядками (`posts-en`, `git add posts/`), мовчазне обрізання історій (документи — 500 рядків, «20 на розширення»), відсутність переходу від висновку до сирих доказів. Окреме завдання вимагало переробити звіт у «підсумок спершу» БЕЗ втрати зібраних даних.

**EVIDENCE** (детальний запис — `security-audit-private/DOCUMENTATION.md`, сесія 7, коміт 8bcac08):
- **Що зроблено (лише `security-audit-private`: `audit_core.ps1` і `tests/`):** модель FULL COLLECTION → NORMALIZED FINDINGS → SUMMARY → GROUPED EVIDENCE → FULL HISTORY ON DEMAND; один автономний HTML (нічого не завантажується й не надсилається); огляд із лічильниками CRITICAL / IMPORTANT / REVIEW / INFORMATIONAL / MODULE ERRORS, 5–10 головних знахідок, план дій із реальних знахідок; 8 категорій; таблиці з пошуком, сортуванням, фільтрами, сторінками 25/50/100/Усі; «Показати докази» веде до точного сирого рядка; перемикач чутливих даних (за замовчуванням сховані); статуси модулів → **AUDIT PARTIAL** і ніколи не «зелений» розділ після збою.
- **Ліцензування:** статус / канал / докази активатора — три окремі питання; KMS або Volume ніколи не є висновком про піратство; неактивований Office = «Office не активовано» (не «піратство»).
- **Докази передачі:** аналіз команд по токенах, впевненість CONFIRMED / PROBABLE / INDICATOR ONLY, кореляція з Prefetch/архівами; `git push` не доводить конкретний файл; Telegram — «точні файли не відновлено»; файл у папці синхронізації ≠ вивантаження.
- **Збереження даних:** нічого корисного не видалено. Видалено лише 4 класи очевидного шуму (USB root hub/host controller у PnP, заглушка монітора без id, дублікати файлів через перекриття коренів, власні script-block події аудиту) — кожен задокументований із обґрунтуванням і тестами. Зняті мовчазні обмеження (документи 500 → усе; «20 на розширення»; чутливі файли в хмарних папках 30 → запобіжник 5000); збережені обмеження показані в примітках таблиць.
- **Реальний прогін на ПК власника знайшов 2 хибні IMPORTANT** (пошуковий фільтр з назвою активатора в історії PowerShell; штатна служба `WdiSystemHost` через неукладений шлях `svchost.exe -k …`) — виправлено й закріплено тестами.
- **Тести (справжній PowerShell 5.1):** 220 тестів — 220 пройшло, 0 впало, 0 пропущено (справжній PowerShell 5.1; той самий набір на LF-блобі, який віддає GitHub, без slow/UI — 184/184) — static, real-run, logic, licensing, transfer (50+ аналізів команд), noise, false-positive, report (на **санітизованій синтетичній фікстурі**: 100 подій USB, ≈1600 записів документів, збій модуля, чутливі дані) і 33 UI-перевірки в реальному браузері (headless Edge). Повний реальний прогін усіх 24 модулів без підвищення: усі 24 модулі виконано за 318 с без підвищення (22 SUCCESS + 2 SKIPPED — Login Events і Executables History), звіт 6,9 МБ, знахідок IMPORTANT/CRITICAL 0.
- **Деплой:** push `d065121..fa84cb7` (fast-forward, без force) у `main` приватного репозиторію `security-audit-private`; blob `audit_core.ps1` на `origin/main` (`e37c779`, 398 294 байти) перевірено через GitHub Contents API = локальний. Бот віддає ядро з `main` приватного репозиторію, окремий деплой бота не потрібен і не виконувався. **Жоден запуск тестової ліцензії не витрачено.** Прогін з підвищенням і шлях «бот → лаунчер → звіт» на реальній ліцензії **не виконувались**.

**DECISION:** рішення №10 без змін — **PAUSE PROMOTION, NOT SUNSET**; просування **PAUSED**. Статус: **REPORT V2 IMPLEMENTED / AWAITING OWNER E2E.** AuditShield **не** позначено VERIFIED IN PRODUCTION і не KEEP/validated; попиту не підтверджено (реальних покупок 0).

**RESULT:** `PROJECT_STATE.md` синхронізовано (AUDITSHIELD STATUS, KNOWN BUGS, COMPLETED WORK, NEXT ACTION). Docs-only: жодних змін коду, збірки, деплою, Telegram-відправок, SEO чи експериментів у цьому репозиторії.

**FOLLOW-UP:** другий owner E2E — **лише за схваленням власника**, з підвищенням, на тій самій тестовій ліцензії (залишалось 2 запуски): перевірити AUDIT COMPLETE/PARTIAL, Login Events і Prefetch у SUCCESS, Office без піратської мітки, відсутність хибних IMPORTANT, працездатність «Показати докази», сховані чутливі дані, відкриття звіту офлайн. Відкриті P1 бота/CRM без змін.

**SECURITY:** запис не містить токенів, значення `ADMIN_CHAT_ID`, платіжних реквізитів, даних клієнтів, ключів чи паролів ліцензій, сирих `jti`/квитків, а також зібраних даних із машини власника; фікстура повністю синтетична.
